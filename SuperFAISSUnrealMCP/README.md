...
| `QueryBank` | Exact top-K by row id, row index, or raw vector; hits carry scores and margins |
...
| `LintBank` | Near-duplicate rows (sampled above a cap, and the report says so) and low-variance dims; cost scales with bank size |

Read/import-only by design: no tool deletes or mutates an existing bank. There is no
